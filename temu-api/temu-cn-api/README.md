---
title: Temu CN API
description: "China cross-border Temu API entry point for inventory, product, supply-chain, and full-managed pricing capabilities within the third-party ERP scope, including supported CN-to-PA migrations."
---
...
| 中国跨境半托库存、货品 | 是 | 迁移后的半托能力通常使用 Partner |
| 中国跨境半托消费者订单、区域履约、区域物流、售后 | 否 | 返回 [根路由](../README.md)，按订单目标站点进入 US/EU/Global |
| US/EU/其他地区本土店铺 | 否 | 使用对应区域模块 |

第三方 ERP 不提供 CN/Partner 营销活动、CN/Partner SKU 供货价查询及半托核价/调价能力。全托核价继续保留，且仅供全托店铺使用，不作为半托核价的替代。
...
以下全托 CN 核价 type 继续保留在 [Price Review](./price-review-api/README.md)，仅用于全托店铺：

| 意图 | 全托 CN |
| --- | --- |
| 分页查询核价单 | `bg.price.review.page.query` |
| 同意核价建议价 | `bg.price.review.confirm` |
| 拒绝核价并重新报价 | `bg.price.review.reject` |
...
- **全托核价/调价**：[Price Review](./price-review-api/README.md)，使用 CN 网关。
...
本模块收录的 PA type 如没有独立详细章节，按迁移表指定的 legacy 对应关系保留其请求与响应契约。调用时使用当前分组索引中的 type、对应 Partner 网关和 PA token；迁移资料不改变上方第三方 ERP 的适用边界。
